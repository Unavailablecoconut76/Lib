      ...
      <div className='p-6'>
        ...
        <form onSubmit={handleRecordBook} >
          <div className='mb-4'>
            ...
            <input type="email" value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="Borrower's email" className='w-full px-4 py-2 border border-black rounded-md' required/>
          </div>
          <div className='flex justify-end epace-x-4'>
            <button className='px-4 py-2 bg-gray-200 rounded-md hover:bg-gray-300' type='button'
            onClick={()=>dispatch(togglerecordBookPopup())}>close</button>
